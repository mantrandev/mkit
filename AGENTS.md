...
## This repository is mkit itself

Two statements in the `MKIT` block below do not apply to this source repository.
Under that block's own `Precedence` rule, these repository rules win:

- **The user of this repository can read code.** The opposite statement applies
  to repositories that install mkit, not the repository that builds it. Use
  normal technical language here.
- **Workflows live in `skills/<name>/SKILL.md`**, not `.mkit/workflows/`. That
  directory is generated by `install.sh` and ignored by git; keeping another
  source copy there would violate mkit's own rules.

Every other rule in the `MKIT` block still applies, including the decision gate.
